...
public class CompMiningExplosive : CompCustomExplosive
{
    ...
    public CompProperties_MiningExplosive MiningProps => (CompProperties_MiningExplosive)props;
    ...
    protected override void Detonate()
    {
        ...
        var cellsByDistance = area.OrderBy(c =>
        {
        ...
        {
            if (!pos.InBounds(parentMap))
            {
                continue;
            }

            var things =
                parentMap.thingGrid.ThingsListAt(pos).ToArray(); // copy required because of collection modification
            ...
            if (breakingPowerRemaining <= 0)
            {
                // ran out of juice, stop breaking
                break;
            }
        }
    ...
    private bool TryAffectThing(Thing thing, Thing explosive, ref float breakingPowerRemaining)
    {
        ...
        if (thing.def.mineable)
        {
            ...
            else if (rockBuildingDef.isNaturalRock)
            {
                ...
                if (rockBuildingDef.mineableThing == null ||
                    !(Rand.Value < explosive.GetStatValue(Resources.Stat.rxExplosiveChunkYield)))
                {
                    return true;
                }

                var rockDrop = ThingMaker.MakeThing(rockBuildingDef.mineableThing);
                rockDrop.stackCount = rockDrop.def.stackLimit == 1 ? 1 : Mathf.CeilToInt(rockBuildingDef.mineableYield);
        ...
        else if (thing.def.plant is { IsTree: true })
        {
            // trees
            ...
            if (culpritPawn != null)
            {
                tree.PlantCollected(placedBy, PlantDestructionMode.Smash);
            }
            else
            {
                tree.Destroy();
    ...
    private static void BreakMineableAndYieldResources(Thing mineable)
    {
        ...
        var resourceYield = Mathf.Max(1,
            Mathf.RoundToInt(building.mineableYield * Find.Storyteller.difficulty.mineYieldFactor));
