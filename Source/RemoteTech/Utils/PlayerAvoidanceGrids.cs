...
public static class PlayerAvoidanceGrids
{
    ...
    public static void AddAvoidanceSource(Thing source, int pathCost)
    {
        ...
        if (!TryGetGridForMap(source.Map.uniqueID, out var grid))
        {
    ...
    {
        if (map == null || !TryGetGridForMap(map.uniqueID, out var grid))
        {
            return null;
        }
    ...
    private static void AssertMap(Thing source)
    {
        if (source.Map == null)
        {
            throw new ArgumentException($"Source thing does not belong to a map: {source}");
        }
    }
