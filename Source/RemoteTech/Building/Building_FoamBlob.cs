...
public class Building_FoamBlob : Building
{
    ...
    {
        return string.Format("FoamBlob_solidify_progress".Translate(),
            100 - Mathf.Ceil(ticksUntilHardened / (float)foamProps.ticksToHarden.max * 100));
    }
    ...
    private void UpdateScale()
    {
        ...
        spriteScaleMultiplier = new Vector2(easedProgress * xScalar, easedProgress * yScalar);
    }
    ...
    private IntVec3 TryFindNearestCellForNewBlob(IntVec3 originalPosition, Map map, int maxDistance)
    {
        ...
        while (cellQueue.Count > 0)
        {
            ...
            if (containsFoam)
            {
                ...
                {
                    if (Mathf.Sqrt(adjacentCell.DistanceToSquared(originalPosition)) > maxDistance ||
                        visitedCells.Contains(adjacentCell))
                    {
                        continue;
                    }
