...
/// <summary>
///     Draws the effective area of a shaped charge, changing color depending on its ability to break any thick mountain
///     roof.
///     Also: highlights any thick mountain roof near the effective area, as well as the effective areas of any other
///     charges
...
public class PlaceWorker_RoofBreakerExplosive : PlaceWorker
{
    ...
    public override void DrawGhost(ThingDef def, IntVec3 center, Rot4 rot, Color ghostCol, Thing thing = null)
    {
        ...
        if (Find.Selector.NumSelected <= 1)
        {
            ...
            foreach (var building in colonistBuildings)
            {
                if (building?.def == def)
                {
                    DrawMatchingEdgesAroundThing(building);
                }
            }
            ...
            foreach (var blueprint in blueprints)
            {
                ...
                {
                    DrawMatchingEdgesAroundThing(blueprint);
                }
            }
        }

        // highlight own effective radius with color-coded effectiveness
        var effectiveRadiusColor =
            RemoteTechUtility.IsEffectiveRoofBreakerPlacement(ownEffectiveRadius, center, map, true)
    ...
    private static void GatherCellsInRadius(IntVec3 centerCell, Map map, float radius,
        Predicate<IntVec3> cellFilter = null)
    {
        cellBuffer.Clear();
        var numCellsInRadius = GenRadial.NumCellsInRadius(radius);
        for (var i = 0; i < numCellsInRadius; i++)
        {
    ...
    private static class OverlayDrawer
    {
        ...
        public static void DrawFieldEdges(List<IntVec3> cells, Color color)
        {
            ...
            var x = currentMap.Size.x;
            var z = currentMap.Size.z;
                ...
                for (var k = 0; k < 4; k++)
                {
                    if (rotNeeded[k])
                    {
                        Graphics.DrawMesh(MeshPool.plane10,
                            intVec.ToVector3ShiftedWithAltitude(AltitudeLayer.MetaOverlays),
