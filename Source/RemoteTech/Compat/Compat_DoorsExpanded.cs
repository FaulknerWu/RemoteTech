...
internal static class Compat_DoorsExpanded
{
    public static void OnDefsLoaded()
    {
        ...
        try
        {
            ...
            {
                throw new Exception($"Expected {doorExpandedTypeName} to extend {nameof(Building)}");
            }

            var openIntField =
                doorExpandedType.GetField(openIntFieldName, BindingFlags.Instance | BindingFlags.NonPublic);
