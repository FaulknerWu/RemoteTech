...
namespace RemoteTech
{
    ...
    public class RemoteTechController
    {
            ...
            ThingDef originalIngredient, ThingDef replacementIngredient, float replacementRatio, float workAmountMultiplier)
        {
            ...
            {
                if (!ingredientCount.filter.Allows(originalIngredient)) continue;
        ...
        private void InjectUpgradeableStatParts()
        {
            try
            {
                ...
                {
                    if (def.comps.Count <= 0) continue;
                    foreach (var comp in def.comps)
                    {
                        if (comp is not CompProperties_Upgrade upgradeProps) continue;
        ...
        private void PrepareReverseBuildingMaterialLookup()
        {
            try
            {
                ...
                foreach (var def in DefDatabase<ThingDef>.AllDefsListForReading)
                {
                    ...
                    {
                        if (compProperties is not CompProperties_BuildGizmo) continue;
                ...
                foreach (var def in DefDatabase<ThingDef>.AllDefsListForReading)
                {
                    ...
                    foreach (var countClass in def.costList)
                    {
                        ...
                        if (materialDef == null || !MaterialToBuilding.TryGetValue(materialDef, out var buildingDefs)) continue;
        ...
        private ThingDef GetDefWithWarning(string defName)
        {
            ...
            if (def == null) Log.Warning($"Could not get ThingDef for Comp injection: {defName}");
            return def;
        }
        ...
        public void LogMessage(string msg) => Log.Message(msg);
    }
}
