...
public class Command_ChannelsKeypad : Command_ChannelsBasic
{
    ...
    public Command_ChannelsKeypad(int selectedChannel, bool switching, Action<int> activateCallback,
        Dictionary<int, List<IWirelessDetonationReceiver>> channelPopulation) : base(selectedChannel, switching,
        activateCallback)
    {
    ...
    public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms _)
    {
        ...
        {
            var channel = i + 1;
            var buttonRect = new Rect(contentRect.x + (i % rowSize * (ButtonSize + buttonSpacing)),
                contentRect.y + (i / (float)rowSize * (ButtonSize + buttonSpacing)), ButtonSize, ButtonSize);
            Graphics.DrawTexture(buttonRect, Resources.Textures.rxUIChannelKeypadAtlas,
                Resources.Textures.KeypadAtlasCoords.Keys[i], ButtonSize, 0, 0, 0);
    ...
    {
        if (hotKey is not { KeyDownEvent: true })
        {
            return false;
        }
    ...
    {
        if (other is not Command_ChannelsKeypad otherKeypad)
        {
            return false;
        }
    ...
    private void TrySelectChargesOnChannel(int channel)
    {
        ...
        charges.ForEach(t =>
        {
            ...
            if (t is ThingComp tc)
            {
                Find.Selector.Select(tc.parent);
            }
        });
    }
