...
public class Graphic_GasCloud : Graphic_Collection
{
    ...
    public override void DrawWorker(Vector3 loc, Rot4 rot, ThingDef thingDef, Thing thing, float extraRotation)
    {
        ...
        var material =
            MaterialPool.MatFrom(new MaterialRequest((Texture2D)defaultMat.mainTexture, defaultMat.shader,
                materialColor));
        var drawPos = cloud.DrawPos;
        var altitude = thing.def.altitudeLayer.AltitudeFor() + (Altitudes.AltInc * cloud.relativeZOrder);
        var matrix = Matrix4x4.TRS(
            new Vector3(drawPos.x + cloud.spriteOffset.x, altitude, drawPos.z + cloud.spriteOffset.y),
            Quaternion.AngleAxis(cloud.spriteRotation, Vector3.up),
            new Vector3(drawSize.x * cloud.spriteScaleMultiplier.x, 0, drawSize.y * cloud.spriteScaleMultiplier.y));
        Graphics.DrawMesh(MeshPool.plane10, matrix, material, 0);
    }

    public override string ToString()
    {
        return $"GasCloud(path={path}, shader={Shader}, color={color}, variants={subGraphics.Length})";
    }
}